/**
 * B2C native store catalog (dev / MVP).
 * Keep in sync with frontend/lib/store-catalog.ts and B2C-Accounting prisma seed products.
 * Versa and other external stores use POST /integrations/store/checkout instead.
 */
...
/** One-click demo cart for E2E: rice + oil bundle (matches early smoke tests). */
export const DEMO_CART_SKUS: Array<{ sku: string; quantity: number }> = [
